...
export default function ExecutorsModal({ isOpen, onClose }: ExecutorsModalProps) {
  ...
  const features = [
    { icon: Shield, text: "Secure & Trusted", color: "text-blue-600" },
    { icon: Zap, text: "Instant Delivery", color: "text-green-600" },
    { icon: Star, text: "Premium Quality", color: "text-purple-600" },
  ]
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed left-1/2 top-1/2 w-[95%] max-w-4xl bg-white rounded-2xl shadow-2xl z-[140] overflow-hidden select-none max-h-[90vh] overflow-y-auto"
        style={{
          opacity: isClosing ? 0 : 1,
          transform: isClosing
      ...
      >
        <div className="p-6 md:p-8">
          ...
          >
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">Choose Your Executor</h2>
              <p className="text-gray-600">Premium script execution tools for all your needs</p>
            </div>
            <Button
              variant="ghost"
              size="icon"
              onClick={handleCloseModal}
              className="text-gray-600 hover:text-gray-900 h-10 w-10 flex-shrink-0"
            >
              <X className="h-5 w-5" />
            </Button>
          </div>
          ...
          >
            <h3 className="text-lg font-bold text-gray-900 mb-4 uppercase tracking-wider">EXECUTORS</h3>

            <div className="space-y-3">
              {executors.map((executor, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200 hover:border-green-300 hover:shadow-md transition-all duration-300 group cursor-pointer"
                  style={{
                    opacity: isClosing ? 0 : 0,
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{executor.description}</p>
                    </div>
                  </div>
          ...
          >
            <Link
              href="/resellers"
              className="flex-1 text-center py-3 px-6 border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all duration-300 transform hover:scale-105"
              onClick={handleCloseModal}
            >
              Compare Prices
            </Link>
            <button
              onClick={handleCloseModal}
              className="flex-1 py-3 px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl transition-all duration-300 transform hover:scale-105"
            >
              Browse Later
            </button>
          </div>
